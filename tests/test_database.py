...
def test_query_tag_list():
    ...
    assert all(result["TagItemID"] == want["TagItemID"])
...
def test_query_insturment_data():
    ...
    want = pd.DataFrame(
        ...
        columns=["TagTimestamp", "TagItemID", "TagValue"],
    )

    assert all(result["TagTimestamp"] == want["TagTimestamp"])
    assert all(result["TagItemID"] == want["TagItemID"])
    assert all(np.isclose(result["TagValue"], want["TagValue"], FLOAT_TOL))
